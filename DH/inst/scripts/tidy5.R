...
tidy <- function(catch, drifts, verbose = FALSE){
  ...
  siteNames = substr(catch$IDCell.per.Trip, 0, 3)
  #loops
    # calculate CPUE per site
    tmp = lapply(1:nrow(catch), #this will run through every row in the BLU_only_depths matrix
           function(x) {
    ...
    # determine if the start or end depth is deeper and save the deeper value
               o <- sort(c(catch$start_depth[x], catch$end_depth[x]))
               num = o[2] - o[1] +1 

    # new dataframe to collect all the data in this iteration of the loop
               this_fish <- data.frame(site = rep(siteNames[x], times = num),
                                       size = rep(catch$size[x], times = num),
                                       depth = o[1]:o[2],
                                       catch = rep(1, times = num))
    # time keeper
               if(verbose) print(x)
               this_fish
  })
